...
function encodeToSmallest(levelValues, levels) {
  // levelValues: array ordered largestΓåÆsmallest (matches form display order)
  // levels: array ordered smallestΓåÆlargest (matches Firestore schema)
...
function decodeFromSmallest(smallest, levels) {
  ...
  // vals is now smallestΓåÆlargest, reverse for display (largestΓåÆsmallest)
  return vals.reverse();
}
...
function renderDashboard(bars) {
    ...
    if (query && currentFilter !== "all") {
      ...
      if (diff > 0) {
        searchHelper.innerHTML = `
          <span>≡ƒöì Found <strong>${diff}</strong> more match${diff === 1 ? '' : 'es'} in other categories.</span>
  ...
  // THEN render bar cards ΓÇö newest first (reverse createdAt asc order)
      ...
      ${getDeadlineMs(bar) ? (() => {
        ...
        return `
          ...
          <div class="card-deadline-label${overdueClass}" data-deadline-ms="${dMs}" data-percent="${percent}" style="margin-top: 10px; font-size: 0.8rem; color: var(--text-muted);">
            ${percent >= 100
              ? `<span class="badge-completed">Γ£ô Completed</span>`
              : `<span class="deadline-text-val">ΓÅ▒ ${label}</span>`
            }
          </div>
        `;
      })() : ''}
    ...
    if (deadlineMs) {
      if (percent >= 100) {
        ...
        if (!labelEl.querySelector('.badge-completed')) {
          labelEl.innerHTML = `<span class="badge-completed">Γ£ô Completed</span>`;
        }
      } else {
        ...
        valSpan.innerHTML = `ΓÅ▒ ${result.label}`;
        if (result.isOverdue) {
...
function openUpdateModal(bar) {
  ...
  if (safeLevels.length === 1) {
    ...
    container.innerHTML = `
      <button type="button" class="stepper-btn" data-action="minus">ΓêÆ</button>
...
async function migrateGuestBarsToFirestore(uid) {
  ...
  if (failCount === 0) {
    // All bars migrated successfully ΓÇö safe to clear local data
    localStorage.removeItem('progress_shelf_bars');
  } else {
    // Partial failure ΓÇö restore guest mode so local data is not lost and user can retry
...
initAuthProtection(async (user) => {
  ...
  if (!isConfigured && !isGuestMode()) {
    if (!document.getElementById("sandbox-banner")) {
      ...
      banner.innerHTML = `<span>Γä╣∩╕Å <strong>Sandbox Mode:</strong> Running locally. Copy <code>firebase-config.template.js</code> to <code>firebase-config.js</code> and fill in credentials to connect Firebase.</span>`;
      ...
      if (cardsGrid.querySelectorAll(".card-skeleton").length > 0) {
        cardsGrid.innerHTML = `
          <div class="empty-state" style="border-color: var(--error);">
            <div class="empty-icon" style="color: var(--error)">ΓÜá∩╕Å</div>
